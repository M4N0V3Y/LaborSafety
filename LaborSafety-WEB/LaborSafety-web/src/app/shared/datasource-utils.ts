...
export function definePredicateColeta<T>(columnsAccept: string[] = null, columnsIgnore: string[] = null) {
    return (data: T, filter: string) => {
        if (columnsAccept && columnsAccept.length > 0) {
            return setForEachColumns<T>(columnsAccept, columnsIgnore, data, filter);
        } else {
            const propsName = Object.getOwnPropertyNames(data);
            return setForEachColumns<T>(propsName, columnsIgnore, data, filter);
        }
    };
}

export function setForEachColumns<T>(props: string[], columnsIgnore: string[], data: T, filter: string) {
    let retornoValid = false;

    props.forEach(c => {
        if ((!columnsIgnore || columnsIgnore.length === 0)
            || (columnsIgnore.filter(x => x.trim().toLowerCase() === c.trim().toLowerCase()).length === 0)) {
            retornoValid = retornoValid || String(data[c]).toString().trim().toLowerCase().indexOf(filter.trim().toLowerCase()) >= 0;
        }
    });
